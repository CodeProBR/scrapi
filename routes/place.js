...
router.get('/country', async function(req, res, next){
    if(req.query.appid && req.query.token && await validate.token(req.query.appid,req.query.token)){
        try {
            let results = await axios.get(`https://servicodados.ibge.gov.br/api/v1/localidades/paises`);
            results = results.data.map(
                obj => {
                    return {
                        "id": obj.id.M49,
                        "code": obj.id['ISO-ALPHA-3'],
                        "name": obj.nome,
                        "region": {
                            "id": obj['sub-regiao'].regiao.id.M49,
                            "name": obj['sub-regiao'].regiao.nome,
                            "subname": obj['sub-regiao'].nome
                        }
...
router.get('/region', async function(req, res, next){
    if(req.query.appid && req.query.token && await validate.token(req.query.appid,req.query.token)){
        try {
            let results = await axios.get(`https://servicodados.ibge.gov.br/api/v1/localidades/regioes`);
            results = results.data.map(
                obj => {
                    return {
                        "id": obj.id,
...
router.get('/microregion', async function(req, res, next){
    if(req.query.appid && req.query.token && await validate.token(req.query.appid,req.query.token)){
        if(req.query.state){
            try{
